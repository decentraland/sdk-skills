...
- **An entity-bound edge does NOT suppress the scene-root broadcast** (measured 2026-08-28). An aimed `press_input` logs the entity-bound hit *and* increments the scene-root counter on the same frame, whether aimed by `x/y/z` or by `entityId`; every successful `click_entity`/`click_at` does the same. The tool description claims otherwise — do not build a scene demo on the suppression until it is settled Explorer-side. Related trap when checking this yourself: a scene measuring "global" input with `inputSystem.isTriggered(action, type)` and **no** entity is not measuring the scene-root broadcast — the SDK answers that form from *every* entity's `PointerEventsResult`, so an entity-bound event satisfies it too. Read `engine.RootEntity` explicitly, and check the scene's own measurement before blaming the client.
...
- **Scene UI receives `PetDown` but not `PetUp`** (measured 2026-08-28). `ui_click` and `ui_drag` on `stack:sdk` fire `onMouseDown` handlers reliably; no `onMouseUp` handler fired on any path, including a `ui_click` aimed straight at an element whose `ui_list` entry advertises `pointerEventTypes: ["PetUp"]`. `ui_drag`'s `info` line names the element it "released" on, but that leg is not delivered — verify a drag by its effect, never by the result line. Scene UI built on mouse-up is currently not drivable.
