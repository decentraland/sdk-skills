...
- **`ui_list`'s `screenRect` is in native backing-store pixels, not screenshot pixels.** On a Retina display the panel's rects ran to ~3424x1926 while `screenshot` returned 1280x720, so normalizing a rect center for `ui_drag`/`click_at` by the screenshot width aims at the wrong place. Derive the native size from the rects themselves (two elements of known on-screen extent give the scale) and divide by that. Worked example: a drag half at `x:1043 y:1281 w:648 h:144` has center `(1367, 1353)` → `fromX: 0.399, fromY: 0.702`.
...
- **The virtual-device path (`device:true`) drives the client UI stack, not UI Toolkit scene panels.** `ui_click device:true` on an SDK element reports that the element observed nothing; use the semantic path for `stack:sdk` and keep the device path for `stack:ugui`.
