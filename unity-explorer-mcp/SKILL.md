...
- **The fan-out is either/or, per action edge: an edge that lands entity-bound never also broadcasts to the scene root.** An aimed `press_input`/`click_entity`/`click_at` on a qualified target increments only the entity's counter; the same edge unaimed (or on an unqualified target) reaches the scene root instead. The split is decided the frame the edge fires — independent of scene tick timing — and an unrelated action pressed the same frame keeps its own broadcast. A scene demo can rely on this contrast; if both counters increment for one edge, that is a client regression. Trap when measuring it: a scene reading `inputSystem.isTriggered(action, type)` with **no** entity is not measuring the scene-root broadcast — the SDK answers that form from *every* entity's `PointerEventsResult`, so an entity-bound event satisfies it too. Read `engine.RootEntity` explicitly, and check the scene's own measurement before blaming the client.
