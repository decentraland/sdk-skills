...
- `screenCoordinates` _(optional Vector2)_ — cursor position in pixels. **Origin is the top-left corner of the screen** (positive Y = down — the same space as `UiTransform`/`UiCanvasInformation`, so cursor coords drop straight into a UI `position`). When the cursor is locked, freezes at the screen center.
- `screenDelta` _(optional Vector2)_ — how many pixels the mouse moved since the last frame. Positive `x` = right, positive `y` = down (top-left origin). **Keeps reporting raw mouse movement while the cursor is locked** — unlike `screenCoordinates` and `worldRayDirection`, which freeze at screen center. This makes `screenDelta` the only way to read mouse movement during pointer lock, and the correct input for mouselook / FPS camera controls (see the **camera-control** skill's mouselook pattern). `screenDelta` is always 0 on mobile (no continuous cursor); `screenCoordinates` and `worldRayDirection` work on both platforms.
