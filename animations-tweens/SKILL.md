...
- `https://github.com/decentraland/sdk7-test-scenes/tree/main/scenes/79,-4-tween-from-current-position` — `Tween.setMove` from `Transform.get(entity).position`: click-to-retarget pads (mid-travel redirection), a "come to me" pad using the player position, a hardcoded-`start` counter-example (visible teleport), and `tweenSystem.tweenCompleted` arrival feedback.
