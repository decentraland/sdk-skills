...
- **Animation appears frozen after porting `anim.stop(); anim.play()` to a hand-rolled SDK7 helper called every frame**: applies specifically when you replace SDK6's `anim.stop(); anim.play()` with a custom SDK7 helper that directly mutates `Animator.states` (`s.playing`, `s.shouldReset`) AND that helper is invoked from a per-frame caller — a system update, or an input/raycast callback that fires every tick the input is held. Each tick rewrites `shouldReset: true` on the active state and the renderer snaps the clip back to frame 0, so the animation visually freezes. SDK6's renderer-side `AnimationState` was lenient about rapid stop/play, hiding the issue during development. **Fix**: track the last-applied clip per entity and short-circuit the helper when unchanged; on a `noLoop + revertToIdle` re-call with the same clip, only refresh the revert timer instead of re-mutating states; clear the tracked clip before re-applying the idle clip from the revert callback. **Does NOT apply** when the helper is called only on a state transition (one-shot `onPointerDown`, edge-triggered state machine), nor when you use `Animator.playSingleAnimation` from a transition rather than a per-frame caller. See the "Custom clip-switch helper called every frame freezes the animation at frame 0" PITFALL in [[animations-tweens]] for the full helper pattern and worked SDK6-port example.
