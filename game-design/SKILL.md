...
| Spectate / observer / director camera | **spectate-mode** | Drop-in module: free or player-following camera, WASD/E/F/1/2 controls, parcel-bounds clamping, player roster. Combine with an admin check (authoritative-server) to restrict who can enter spectate mode. |
