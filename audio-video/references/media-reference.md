...
| `registerAudioPlaybackEntity(entity, cb)` | `cb(event)` once per scene frame with the newest report, position updates included. The renderer writes one whenever the playhead moves, so every render frame while a clip plays. |
| `registerAudioPlaybackSampleEntity(entity, cb)` | `cb({ report, sceneTime, offset })` — the same delivery, already resolved against the scene clock at the sampling tick. Prefer it. |
| `removeAudioPlaybackSampleEntity(entity)` | Unregisters the sample callback. |
