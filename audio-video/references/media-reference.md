...
| `registerAudioPlaybackEntity(entity, cb)` | `cb(event)` on every report, including the periodic position reports while playing (Unity explorer: every 15 ticks, about twice a second). |
...
audioEventsSystem.registerAudioPlaybackEntity(entity, (report) => {
  // every report; while playing: report.tickNumber + report.currentOffset (+ clipLength when known)
})
...
`AudioSource.currentTime` is a write-only seek (reading it never gives the playhead) and renderers start a clip 100–250 ms after being asked, so the reports are the only source of truth for where the audio is. Correlate each report by `tickNumber` against a per-tick history of the scene clock — comparing against the clock at processing time is off by the transport delay.
...
const audioNowMs = () => (lagMs === undefined ? undefined : clockMs - lagMs)
```

Fallback: renderers without the feature (today everything but the Unity explorer) never set `currentOffset`, so `lagMs` and `getAudioPlayback` stay `undefined` — use a fixed lead (~150 ms) or manual calibration and never hard-block on a report. Needs an `@dcl/sdk` release containing js-sdk-toolchain [#1624](https://github.com/decentraland/js-sdk-toolchain/pull/1624); check the scene's pin first. Rules and gate in SKILL.md.
